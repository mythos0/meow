...
console.log('— equipping all 48 hat × dress combos through the real IPC —');
const HATS = [null, 'pumpkin', 'santa', 'flower', 'shades', 'tophat', 'crown', 'bow'];
const DRESSES = [null, 'red', 'blue', 'pink', 'midnight'];
...
for (const hat of HATS) {
  for (const dress of DRESSES) {
    ...
    if ((hat === null || hat === 'tophat' || hat === 'crown') && shot < 12) {
      await cat.screenshot({ path: path.join(OUT, `equip-${shot++}-${hat || 'nohat'}-${dress || 'nodress'}.png`) });
    }
  }
}
ok('equip: all 40 combos (8 hats × 5 dresses) applied live with zero renderer errors', comboApplied === 40 && comboErrors === 0,
  `${comboApplied}/40 applied, ${comboErrors} failures — ${comboFails.slice(0, 8).join(' ; ')}`);
