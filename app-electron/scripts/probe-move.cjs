// per-frame setPosition cost probe + dirty-rect at region size
const { app, BrowserWindow } = require('electron');
app.disableHardwareAcceleration();
app.commandLine.appendSwitch('in-process-gpu');
const SCEN = process.env.PROBE_SCEN || 'E';   // E=move60 F=move30 G=dirtyrect
const W = 480, H = 434;
...
app.whenReady().then(() => {
  ...
  setTimeout(() => {
    ...
    const ticks = () => { let s = 0; for (const line of execSync('ps -eo comm,time --no-headers', { encoding: 'utf8' }).split('\n')) { if (!/electron/i.test(line)) continue; const m = line.trim().match(/(\d+):(\d+)$/); if (m) s += (+m[1]) * 60 + (+m[2]); } return s; };
    const s0 = ticks(); const t0 = Date.now();
    let moving = MOVE > 0;
    const mover = moving ? setInterval(() => {
      const b = win.getBounds();
      const nx = b.x >= 200 ? 0 : b.x + 2, ny = b.y >= 200 ? 0 : b.y + 2;
      win.setPosition(nx, ny, false);
    }, MOVE) : null;
    setTimeout(() => {
      const s1 = ticks(); const wall = (Date.now() - t0) / 1000;
      if (mover) clearInterval(mover);
      console.log(`PROBE SCEN=${SCEN} cpu=${(((s1 - s0) / wall) * 100).toFixed(1)}%_of_one_core`);
      app.exit(0);
    }, 8000);
  }, 3000);
});
