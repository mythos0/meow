...
const app = spawn(path.join(ROOT, 'node_modules/.bin/electron'), ['.', '--remote-debugging-port=9232', '--no-sandbox', '--disable-gpu'], { cwd: ROOT, env: { ...process.env }, stdio: ['ignore','pipe','pipe'] });
for (let i = 0; i < 60; i++) { try { const r = await fetch('http://127.0.0.1:9232/json/version'); if (r.ok) break; } catch {} await new Promise(r => setTimeout(r, 500)); }
...
const out = await cat.evaluate(async () => {
  ...
  Object.defineProperty(c, 'x', {
    ...
    get() { return realX; },
    set(v) {
      if (Math.abs(v - realX) > 100 && log.length < 3) {
        log.push({ from: Math.round(realX), to: Math.round(v), stack: (new Error().stack || '').split('\n').slice(1, 5).join(' | ') });
      }
      realX = v;
    },
  });
  c._jump = null; c.onPlatform = null; c._enter('idle', 1);
  c.x = b.x + 1200; log.length = 0;
  const t0 = Date.now();
