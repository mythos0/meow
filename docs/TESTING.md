...
| E2E v3.7 | 9 | real Electron under Xvfb + CDP (`e2e-v37.mjs`) | **hop does not flap the region** (jumping-bug fix) → cat never drawn outside the canvas during slides → slides still happen while strolling → companion kitten leashed back and kept visible → **quick tap plays a real meow + heart** → **v3.10: a call app does NOT hide the cat, does NOT pause the render loop, `__catVisible` stays true** |
...
```bash
cd app-electron
npm test                        # unit + visual (351 checks)
