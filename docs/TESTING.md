...
| E2E v3.11 | 30 | real Electron under Xvfb + CDP (`e2e-v311.mjs`) | **the walking-flicker test done properly**: 90 consecutive canvas frames captured while the cat walks — the cat is visible in EVERY frame, frame-to-frame motion stays continuous (max bbox jump ≤ 26px measured 7.1px), and the window issues ZERO region moves during the stroll → **drag-follow**: the cat hauled 620px outside the old region stays ON the canvas and visible in every sample, the window follows, and the drop lands exactly where released (the cursor is now driven by main) → **sound spec**: quick click = `meow_single`, double-click = classic voice, ambient scheduler armed, master toggle silences everything → **zone selector**: the screenshot-style overlay opens with the display union, a drawn rect lands in the zone list workArea-relative → **never-stop contract**: helper windows closed, second launcher exits quietly, first cat survives, typing hook enabled at boot |
| E2E v3.12 dual | 29 | real Electron under Xvfb + CDP (`e2e-dual.mjs`, simulated TWO displays) | **the 2nd-monitor release, proven on a simulated dual display (3200px union)**: settings expose both work areas, the brain roams the union, the lane stays display-sized → **main-driven drag onto display 2**: the cat crosses the seam with the window following, visible in EVERY sample, drops exactly where released → **auto-walk onto display 2**: the lane HOP fires at the lane edge and the stroll continues onto the next monitor (21/21 visible samples) → **no footstep sounds while walking** → **zone base fix**: a rect drawn on display 2 stores union-relative → **double click = classic meow, opens NOTHING** → **sound toggles**: single-click/double-click/master gates + click-vs-ambient mutual exclusion + all five toggles present in the Sounds page → **QUIT GATE**: a non-explicit `app.quit()` is blocked, journaled with a stack trace, and the cat lives on |

Run:

```bash
cd app-electron
npm test                        # unit + visual (395 checks)
...
node scripts/e2e-dual.mjs       # v3.12 dual-display E2E (simulated 2 monitors)
```
